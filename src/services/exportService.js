...
export async function exportToPowerPoint(slides, templateFile = null) {
  try {
    ...
    if (templateFile) {
      ...
      // Preparar datos incluyendo textAreas para reemplazo preciso
      const exportData = {
        slides: slides.map((slide, index) => ({
          type: slide.type,
          content: slide.content,
          // Incluir textAreas para reemplazo preciso por coordenadas
          textAreas: slide.layout?.textAreas || slide.textAreas || []
        }))
      }
      
      console.log('📤 Datos a enviar:', JSON.stringify(exportData, null, 2))
